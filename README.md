...
3. `tk.topics.by` provides a list of data frames, each offering a specific splice of results against one of the columns of metadata. For example, the **shakespeare.csv** data set will offer `tk.topics.by$title`, `tk.topics.by$year`, `tk.topics.by$genre`, and `tk.topics.by$dynasty`, among others. After the dollar sign, RStudio will offer the option to choose from a menu of options, which is useful for discovery.
![image showing RStudio's completion suggestions](http://jmclawson.com/topickit/rstudio-autocomplete-2.png "RStudio's suggested completions are useful for discovering available views.")
